...
/*   Updated: 2023/05/09 19:42:12 by nkhoudro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void add_player(t_map *map)
{
	if (map->imp_pos == 1)
		...
		map->img_ptr_p = mlx_xpm_file_to_image(map->mlx_ptr, "left_mario.xpm", &(map->width), &(map->height));
	else if (map->imp_pos == 3)
		map->img_ptr_p = mlx_xpm_file_to_image(map->mlx_ptr, "right_mario.xpm", &(map->width), &(map->height));
	map->imp_pos = 0;
